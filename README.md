...
- **Ollama** как локальный провайдер эмбеддингов (модель `nomic-embed-text`),
...
### Запуск Desktop‑клиента (опционально)

```shell
./gradlew :composeApp:run
```

Клиент общается с сервером по REST `/api/sessions/...` и не знает о RAG/Ollama напрямую – он просто отображает ответы ассистента.
